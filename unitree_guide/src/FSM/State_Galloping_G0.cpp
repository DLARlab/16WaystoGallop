...
#include <ros/ros.h>
#include <iostream>
#include "FSM/State_Galloping_G0.h"
...
State_Galloping_G0::State_Galloping_G0(CtrlComponents *ctrlComp)
             :FSMState(ctrlComp, FSMStateName::GALLOPING_G0, "Galloping_G0"), 
...
State_Galloping_G0::~State_Galloping_G0(){
    delete _gait;
}

void State_Galloping_G0::enter(){
    _ctrlComp->motioncounter =0;
    ...
    file_posTorso10.open("posTorso10.txt");
    file_posTorso11.open("posTorso11.txt");
    file_posTorso12.open("posTorso12.txt");
...
file_posTorso11.close();
file_posTorso12.close();
...
FSMStateName State_Galloping_G0::checkChange(){
...
void State_Galloping_G0::run(){
 ...
 std::cout << "_G2B_RotMat: " << _G2B_RotMat << std::endl;
  ...
  if(_vCmdUser(0) >5 && ( *_contact)(0) == 0 && ( *_contact)(3) == 0 && _ctrlComp->motioncounter >= 110500 &&  _ctrlComp->motioncounter < 111000){
        _period = 0.226;
        _stancePhaseRatio = 0.22;
        _bias = Vec4(0, 0, 0.52, 0.52);
    }
    // else if(_vCmdUser(0) >2.5 &&( *_contact)(0) == 0 && ( *_contact)(3) == 0 && _ctrlComp->motioncounter >= 11000 &&  _ctrlComp->motioncounter < 11500){
    //     _period = 0.226;
    //     _stancePhaseRatio = 0.22;
    //     _bias = Vec4(0, 0, 0.54, 0.54);
    // }
    // else if(_vCmdUser(0) >2.5 && ( *_contact)(0) == 0 && ( *_contact)(3) == 0 && _ctrlComp->motioncounter >= 11500 &&  _ctrlComp->motioncounter < 12000){
    //     _period = 0.226;
    //     _stancePhaseRatio = 0.22;
    //     _bias = Vec4(0, 0, 0.56, 0.56);
    // }
    // else if(_vCmdUser(0) >2.5 && ( *_contact)(0) == 0 && ( *_contact)(3) == 0 && _ctrlComp->motioncounter >= 12000 &&  _ctrlComp->motioncounter < 12500){
    //     _period = 0.226;
    //     _stancePhaseRatio = 0.22;
    //     _bias = Vec4(0, 0, 0.58, 0.58);
    // }
    // else if(_vCmdUser(0) >2.5 && ( *_contact)(0) == 0 && ( *_contact)(3) == 0 && _ctrlComp->motioncounter >= 12500 &&  _ctrlComp->motioncounter < 13000){
    //     _period = 0.226;
    //     _stancePhaseRatio = 0.22;
    //     _bias = Vec4(0, 0, 0.6, 0.6);
    // }
    else {//} ( _ctrlComp->motioncounter < 500){
        _period = 0.25;
        _stancePhaseRatio = 0.5;
        _bias = Vec4(0, 0.1, 0.51, 0.61); 
    } 
...
bool State_Galloping_G0::checkStepOrNot(){
...
// void State_Galloping_G0::setHighCmd(double vx, double vy, double wz){
//     _vbCmdBody(0) = vx;
//     _vbCmdBody(1) = vy;
//     _vbCmdBody(2) = 0; 
//     _dYawCmd = wz;
...
void State_Galloping_G0::getUserCmd(){
    /* Movement */
    ...
    if ( counterDuStance < 20){
          _vbCmdBodyDhc(0) = counterDuStance * 0.01 + 0.05;
          _vCmdUser(0) = _vbCmdBodyDhc(0);
        }
    else if ( counterDuStance <= 100){
        _vbCmdBodyDhc(0) = (counterDuStance - 20) * 0.008 + 20 * 0.01 + 0.05;
        _vCmdUser(0) = _vbCmdBodyDhc(0);
    }
    else if ( counterDuStance > 100){
      _vbCmdBodyDhc(0) = (counterDuStance - 100) * 0.006 + 80 * 0.008 + 20 * 0.01 + 0.05;
      _vCmdUser(0) = _vbCmdBodyDhc(0);
    }
    
    // else if ( counterDuStance > 300){
    //   _vbCmdBodyDhc(0) = 2.09;
    //   _vCmdUser(0) = _vbCmdBodyDhc(0);
    // }
...
void State_Galloping_G0::calcCmd(){
    /* Movement */
        ...
        //     _rollCmd = (- 0.05 * _vCmdUser(0) ) + (0.05 + 0.015 * _vCmdUser(0)) * cos((2 * M_PI/strideIteration) * counterUse - M_PI/(15 - 6.5 *  _vCmdUser(0))); // calculate the desired pitch angle
        // }
        ...
        //      _rollCmd = -0.1 - 0.01 * ( _vCmdUser(0) - 2) + (0.08 + 0.03 *(_vCmdUser(0) - 2) ) * cos((2 * M_PI/strideIteration) * counterUse- M_PI/(2 - 0.1 *  (_vCmdUser(0) - 2))); // calculate the desired pitch angle
        // }
        // else{
        //     _pitchCmd = _lowState->getPitch();
        //     _rollCmd = _lowState->getRoll();
        // }   
         _pitchCmd = 0.0; 
    if((*_contact)(0) == 1 || (*_contact)(1) == 1){
        ...
        std::cout << "phaseCounter" << phaseCounter << ": " <<  counterUse <<": " << flightDetector << std::endl;
    ...
    std::cout << "counterDuStance: " << counterDuStance << std::endl;
    ...
    _Rd = rotz(_yawCmd) * roty(_pitchCmd)* rotx(_rollCmd); // desired rotation 2.5matrix from desired yaw and pitch angle
    ...
    if(file_posTorso10.is_open()){file_posTorso10<< _lowState->getRoll() << "\n";}
    if(file_posTorso11.is_open()){file_posTorso11<< _lowState->getPitch() << "\n";}
    if(file_posTorso12.is_open()){file_posTorso12<< _lowState->getYaw() << "\n";}
 ...
 */
void State_Galloping_G0::calcTau(){
    // if((*_contact)(0) == 1 || (*_contact)(1) == 1){
...
////////////////////////////////////////////////////////
    ...
    if (counter > 2000000){
        if((*_contact)(0) == 1 || (*_contact)(1) == 1){
            ...
            if((*_phase)(1) > 0.39){
                for (int i = 0; i < 2; i++){
                    _forceFeetGlobalAdd[i] =  350 * (_wPosFhc(2) - _wfhcd(2))+ 1.0 * (_VelFhc(2) - 0);
                }
                _Kpp = Vec3(150, 70, 200).asDiagonal();
                _Kdp = Vec3(20, 20, 30).asDiagonal();
            ...
            if((*_phase)(2) > 0.39){
                for (int i = 2; i < 4; i++){
                    _forceFeetGlobalAdd[i] =  1000* (_wPosRhc(2) - _wrhcd(2))+ 1.0 * (_VelRhc(2) - 0);
                }
                _Kpp = Vec3(70, 70, 70).asDiagonal();
                _Kdp = Vec3(20, 20, 20).asDiagonal();
                _kpw = Vec3(400, 800, 400).asDiagonal();
        ...
        if((*_contact)(0) == 1 || (*_contact)(1) == 1){
            ...
            if((*_phase)(2) > 0.39){
                for (int i = 2; i < 4; i++){
                    _forceFeetGlobalAdd[i] =  100 * (_wPosRhc(2) - _wrhcd(2))+ 0.0 * (_VelRhc(2) - 0);
                }
                _Kpp = Vec3(70, 70, 70).asDiagonal();
                _Kdp = Vec3(20, 20, 20).asDiagonal();
                _kpw = Vec3(400, 100, 400).asDiagonal();
 ...
 * 
 */
void State_Galloping_G0::calcQQd(){
...
Vec3 State_Galloping_G0::xProd(Vec3 vec1, Vec3 vec2){
    Vec3 vec3;
...
void State_Galloping_G0::plotData(std::vector<std::vector<double>>& desd_data, std::vector<std::vector<double>>& actual_data){
...
void State_Galloping_G0::plotSubPlot(std::vector<std::vector<double>>& desd_data, std::vector<std::vector<double>>& actual_data, int legNum){
...
void State_Galloping_G0::plotGRF(std::vector<std::vector<double>>& desd_data, std::vector<std::vector<double>>& actual_data){
...
void State_Galloping_G0::plotXZ(std::vector<std::vector<double>>& desd_data, std::vector<std::vector<double>>& actual_data){
