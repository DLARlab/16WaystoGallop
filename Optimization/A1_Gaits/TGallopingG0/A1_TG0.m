...
if LOAD
    ...
    [sys, domains, guards] = load_behavior(robot, LOAD_PATH,'type','TGallopingG0');
else
    robot.configureDynamics('DelayCoriolisSet',OMIT_CORIOLIS,'OmitCoriolisSet',OMIT_CORIOLIS); 
    [sys, domains, guards] = load_behavior(robot,[], 'type','TGallopingG0');
end
%plot(sys.Gamma)
...
num_grid.stance_FRRL = 8;
nlp = HybridTrajectoryOptimization('A1_TG0',sys,num_grid,[],'EqualityConstraintBoundary',1e-3);
...
nlp.Phase(3).Plant.UserNlpConstraint  = @NLPSetUp.Constraints.FRFLConstraint;   
...
nlp.Phase(15).Plant.UserNlpConstraint = @NLPSetUp.Constraints.FRRLConstraint;  
