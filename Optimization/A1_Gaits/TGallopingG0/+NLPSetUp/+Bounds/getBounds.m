...
model_bounds.states.x.lb(1:3) = [-10,-0.05, 0];
model_bounds.states.x.ub(1:3) = [+10,+0.05, 3];
...
model_bounds.states.x.ub([8 11 14 17]) = deg2rad(+120); % 240
...
%% FRRL stance phase
bounds.stance_FRRL = model_bounds;

% virtual constraints 
bounds.stance_FRRL.time.t0.lb = 0; 
bounds.stance_FRRL.time.t0.ub = 0;  
bounds.stance_FRRL.time.t0.x0 = 0;  

bounds.stance_FRRL.time.tf.lb = 0; 
bounds.stance_FRRL.time.tf.ub = 1;  
bounds.stance_FRRL.time.tf.x0 = 0.2;  

bounds.stance_FRRL.time.duration.lb = 0.03; 
bounds.stance_FRRL.time.duration.ub = 1;  
bounds.stance_FRRL.time.duration.x0 = 0.2;  

bounds.stance_FRRL.time.kp = 150;
bounds.stance_FRRL.time.kd = 20;

% forces
bounds.stance_FRRL.inputs.ConstraintWrench.fFRfoot.lb = -10000;
bounds.stance_FRRL.inputs.ConstraintWrench.fFRfoot.ub = 10000;
bounds.stance_FRRL.inputs.ConstraintWrench.fFRoot.x0 = 100;

bounds.stance_FRRL.inputs.ConstraintWrench.fRLfoot.lb = -10000;
bounds.stance_FRRL.inputs.ConstraintWrench.fRLfoot.ub = 10000;
bounds.stance_FRRL.inputs.ConstraintWrench.fRLfoot.x0 = 100;

% contacts
bounds.stance_FRRL.params.pFRfoot.lb = [-10;-10;0];
bounds.stance_FRRL.params.pFRfoot.ub = [+10;+10;0];

bounds.stance_FRRL.params.pRLfoot.lb = [-10;-10;0];
bounds.stance_FRRL.params.pRLfoot.ub = [+10;+10;0]; 

bounds.stance_FRRL.params.atime.lb = -100*ones(6*12,1); % 5th order polynomial * 12 joints
bounds.stance_FRRL.params.atime.ub =  100*ones(6*12,1);
bounds.stance_FRRL.params.atime.x0 =    zeros(6*12,1);

bounds.stance_FRRL.params.ptime.lb = [bounds.stance_FRRL.time.tf.lb, bounds.stance_FRRL.time.t0.lb];
bounds.stance_FRRL.params.ptime.ub = [bounds.stance_FRRL.time.tf.ub, bounds.stance_FRRL.time.t0.ub];
bounds.stance_FRRL.params.ptime.x0 = [bounds.stance_FRRL.time.tf.x0, bounds.stance_FRRL.time.t0.x0];

%% FLRR stance phase
bounds.stance_FLRR = model_bounds;

% virtual constraints 
bounds.stance_FLRR.time.t0.lb = 0; 
bounds.stance_FLRR.time.t0.ub = 0;  
bounds.stance_FLRR.time.t0.x0 = 0;  

bounds.stance_FLRR.time.tf.lb = 0; 
bounds.stance_FLRR.time.tf.ub = 1;  
bounds.stance_FLRR.time.tf.x0 = 0.2;  

bounds.stance_FLRR.time.duration.lb = 0.03; 
bounds.stance_FLRR.time.duration.ub = 1;  
bounds.stance_FLRR.time.duration.x0 = 0.2;  

bounds.stance_FLRR.time.kp = 150;
bounds.stance_FLRR.time.kd = 20;

% forces
bounds.stance_FLRR.inputs.ConstraintWrench.fFLfoot.lb = -10000;
bounds.stance_FLRR.inputs.ConstraintWrench.fFLfoot.ub = 10000;
bounds.stance_FLRR.inputs.ConstraintWrench.fFLoot.x0 = 100;

bounds.stance_FLRR.inputs.ConstraintWrench.fRRfoot.lb = -10000;
bounds.stance_FLRR.inputs.ConstraintWrench.fRRfoot.ub = 10000;
bounds.stance_FLRR.inputs.ConstraintWrench.fRRfoot.x0 = 100;

% contacts
bounds.stance_FLRR.params.pFLfoot.lb = [-10;-10;0];
bounds.stance_FLRR.params.pFLfoot.ub = [+10;+10;0];

bounds.stance_FLRR.params.pRRfoot.lb = [-10;-10;0];
bounds.stance_FLRR.params.pRRfoot.ub = [+10;+10;0]; 

bounds.stance_FLRR.params.atime.lb = -100*ones(6*12,1); % 5th order polynomial * 12 joints
bounds.stance_FLRR.params.atime.ub =  100*ones(6*12,1);
bounds.stance_FLRR.params.atime.x0 =    zeros(6*12,1);

bounds.stance_FLRR.params.ptime.lb = [bounds.stance_FLRR.time.tf.lb, bounds.stance_FLRR.time.t0.lb];
bounds.stance_FLRR.params.ptime.ub = [bounds.stance_FLRR.time.tf.ub, bounds.stance_FLRR.time.t0.ub];
bounds.stance_FLRR.params.ptime.x0 = [bounds.stance_FLRR.time.tf.x0, bounds.stance_FLRR.time.t0.x0];
