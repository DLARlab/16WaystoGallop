...
OMIT_CORIOLIS = true; % drop veloci terms
...
if LOAD
    ...
    [sys, domains, guards] = load_behavior(robot, LOAD_PATH,'type','TGallopingG2');
else
    robot.configureDynamics('DelayCoriolisSet',OMIT_CORIOLIS,'OmitCoriolisSet',OMIT_CORIOLIS); 
    [sys, domains, guards] = load_behavior(robot,[], 'type','TGallopingG2');
end
%plot(sys.Gamma)
                                        
%% 4- Create Optimization Problem %
% Specifying the Number of collocation points in the stance phase and flight phase %
  num_grid.stance_FR = 10; 
num_grid.stance_FRFL = 5;
  num_grid.stance_FL = 10;
    num_grid.flight1 = 5;
  num_grid.stance_RR = 10; 
num_grid.stance_RRRL = 5;
  num_grid.stance_RL = 10;
    num_grid.flight2 = 5;
nlp = HybridTrajectoryOptimization('A1_TGallopingG2',sys,num_grid,[],'EqualityConstraintBoundary',1e-3);
...
nlp.Phase(9).Plant.UserNlpConstraint  = @NLPSetUp.Constraints.RRConstraint;     
...
nlp.Phase(13).Plant.UserNlpConstraint = @NLPSetUp.Constraints.RLConstraint;      
...
% Compile Constraint:
          compileConstraint(nlp, 1, {'Yaw0'}, EXPORT_PATH, 'dynamics_equation');
...
%% 10- NLP Settings
average_velocity = 2;
nlp.Phase(7).updateConstrProp('average_velocity_entire_stride', 'last','lb', [average_velocity-0.01;0], 'ub', [average_velocity+0.01;0]); 
...
gait = NLPSetUp.Utilities.parse(nlp, sol);
% ----save---- %
% save([pwd,'\Velocity_',num2str(average_velocity),Specs,'.mat'],'nlp','gait','info','sol','bounds')
