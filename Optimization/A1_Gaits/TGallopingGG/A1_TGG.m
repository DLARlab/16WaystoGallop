...
if LOAD
    ...
    [sys, domains, guards] = load_behavior(robot, LOAD_PATH,'type','TGallopingGG');
else
    robot.configureDynamics('DelayCoriolisSet',OMIT_CORIOLIS,'OmitCoriolisSet',OMIT_CORIOLIS); 
    [sys, domains, guards] = load_behavior(robot,[], 'type','TGallopingGG');
end
%plot(sys.Gamma)
...
num_grid.stance_FLRR = 8;
  num_grid.stance_FL = 8; 
num_grid.stance_FRFL = 8;
  num_grid.stance_FR = 8;
     num_grid.flight = 5;
nlp = HybridTrajectoryOptimization('A1_TGallopingGG',sys,num_grid,[],'EqualityConstraintBoundary',1e-3);

%% 5- Add Domain (Phase) Specific Constraints %%
nlp.Phase(1).Plant.UserNlpConstraint  = @NLPSetUp.Constraints.RLConstraint;     
nlp.Phase(2).Plant.UserNlpConstraint  = @NLPSetUp.Constraints.EventConstraint;     
nlp.Phase(3).Plant.UserNlpConstraint  = @NLPSetUp.Constraints.RRRLConstraint;   
nlp.Phase(4).Plant.UserNlpConstraint  = @NLPSetUp.Constraints.EventConstraint;  
nlp.Phase(5).Plant.UserNlpConstraint  = @NLPSetUp.Constraints.RRConstraint;      
nlp.Phase(6).Plant.UserNlpConstraint  = @NLPSetUp.Constraints.EventConstraint;     
nlp.Phase(7).Plant.UserNlpConstraint  = @NLPSetUp.Constraints.FLRRConstraint;    
nlp.Phase(8).Plant.UserNlpConstraint  = @NLPSetUp.Constraints.EventConstraint;      
nlp.Phase(9).Plant.UserNlpConstraint  = @NLPSetUp.Constraints.FLConstraint;     
nlp.Phase(10).Plant.UserNlpConstraint = @NLPSetUp.Constraints.EventConstraint;     
nlp.Phase(11).Plant.UserNlpConstraint = @NLPSetUp.Constraints.FRFLConstraint;   
nlp.Phase(12).Plant.UserNlpConstraint = @NLPSetUp.Constraints.EventConstraint;  
nlp.Phase(13).Plant.UserNlpConstraint = @NLPSetUp.Constraints.FRConstraint;      
nlp.Phase(14).Plant.UserNlpConstraint = @NLPSetUp.Constraints.EventConstraint;     
nlp.Phase(15).Plant.UserNlpConstraint = @NLPSetUp.Constraints.FlightConstraint;   % flight2
...
% Compile Constraint:
%           compileConstraint(nlp, 15, {'average_velocity_entire_stride'}, EXPORT_PATH, 'dynamics_equation');

% Compile Cost Function:  
              compileObjective(nlp, 15, {'Torque'}, EXPORT_PATH,'dynamics_equation'); 
...
%% 13- Variables, Constraints, and Cost log %%
...
%% 14- Ploting The Torque and Trajectory of Each Joint%%
