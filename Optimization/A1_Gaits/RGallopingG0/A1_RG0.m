...
if LOAD
    ...
    [sys, domains, guards] = load_behavior(robot, LOAD_PATH,'type','RGallopingG0');
else
    robot.configureDynamics('DelayCoriolisSet',OMIT_CORIOLIS,'OmitCoriolisSet',OMIT_CORIOLIS); 
    [sys, domains, guards] = load_behavior(robot,[], 'type','RGallopingG0');
end
%plot(sys.Gamma)
...
num_grid.stance_FLRL = 8;
  num_grid.stance_RL = 8; 
num_grid.stance_RRRL = 8;
  num_grid.stance_RR = 8;
num_grid.stance_FRRR = 8;
nlp = HybridTrajectoryOptimization('A1_RG0',sys,num_grid,[],'EqualityConstraintBoundary',1e-3);
...
nlp.Phase(15).Plant.UserNlpConstraint = @NLPSetUp.Constraints.FRRRConstraint;  
