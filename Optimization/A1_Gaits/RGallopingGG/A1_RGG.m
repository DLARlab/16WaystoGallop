...
if LOAD
    ...
    [sys, domains, guards] = load_behavior(robot, LOAD_PATH,'type','RGallopingGG');
else
    robot.configureDynamics('DelayCoriolisSet',OMIT_CORIOLIS,'OmitCoriolisSet',OMIT_CORIOLIS); 
    [sys, domains, guards] = load_behavior(robot,[], 'type','RGallopingGG');
end
%plot(sys.Gamma)
...
num_grid.stance_FRRR = 8;
  num_grid.stance_FR = 8; 
num_grid.stance_FRFL = 8;
  num_grid.stance_FL = 8;
     num_grid.flight = 5;
nlp = HybridTrajectoryOptimization('A1_RGallopingGG',sys,num_grid,[],'EqualityConstraintBoundary',1e-3);
...
nlp.Phase(7).Plant.UserNlpConstraint  = @NLPSetUp.Constraints.FRRRConstraint;    
nlp.Phase(8).Plant.UserNlpConstraint  = @NLPSetUp.Constraints.EventConstraint;      
nlp.Phase(9).Plant.UserNlpConstraint  = @NLPSetUp.Constraints.FRConstraint;     
...
nlp.Phase(13).Plant.UserNlpConstraint = @NLPSetUp.Constraints.FLConstraint;      
...
% Compile Constraint:
          compileConstraint(nlp, 15, {'SwingFootContactVelocity_RL'}, EXPORT_PATH, 'dynamics_equation');
