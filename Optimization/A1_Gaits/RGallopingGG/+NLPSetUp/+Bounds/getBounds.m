...
model_bounds.states.x.lb(1:3) = [-10,-0.5, 0];
model_bounds.states.x.ub(1:3) = [+10,+0.5, 4];

% torso [yaw,pitch,roll] position %
model_bounds.states.x.lb(4:6) = deg2rad([-20,-20,-20]);
model_bounds.states.x.ub(4:6) = deg2rad([+20,+20,+20]);
...
model_bounds.states.x.ub([8 11 14 17]) = deg2rad(+90); % 240  % 90

% calf joint angles -154.5 deg (shortest) to -52.5 deg (longest)
model_bounds.states.x.lb([9 12 15 18]) = deg2rad(-120); % -154.5  % 120
model_bounds.states.x.ub([9 12 15 18]) = deg2rad(-52.5);  % -52.5

%% Multi-Domain Constraints %%

% average velocity in x (forward) direction
bounds.average_velocity.lb = [+0.9;-0];   
bounds.average_velocity.ub = [+1.1;+0];  
...
% bounds.stance_RL.params.pRRfoot.x0 = [-0.157538;-0.042628;0];
...
% bounds.stance_FRRR.params.pRRfoot.x0 = [-0.157538;-0.042628;0];

bounds.stance_FRRR.params.pRRfoot.lb = [-10;-10;0];
bounds.stance_FRRR.params.pRRfoot.ub = [+10;+10;0]; 
% bounds.stance_FRRR.params.pRLfoot.x0 = [-0.157538;0.047;0];
...
bounds.flight.time.tf.lb = 0;
bounds.flight.time.tf.ub = 1;
bounds.flight.time.tf.x0 = 0.2;

bounds.flight.time.duration.lb = 0.03;
bounds.flight.time.duration.ub = 1;
bounds.flight.time.duration.x0 = 0.2;

bounds.flight.time.kp = 150;
bounds.flight.time.kd = 15;

bounds.flight.params.atime.lb = -100*ones(6*12,1); % 5th order polynomial * 12 joints
bounds.flight.params.atime.ub =  100*ones(6*12,1);
