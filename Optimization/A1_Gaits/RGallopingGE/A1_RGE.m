...
if LOAD
    ...
    [sys, domains, guards] = load_behavior(robot, LOAD_PATH,'type','RGallopingGE');
else
    robot.configureDynamics('DelayCoriolisSet',OMIT_CORIOLIS,'OmitCoriolisSet',OMIT_CORIOLIS); 
    [sys, domains, guards] = load_behavior(robot,[], 'type','RGallopingGE');
end
%plot(sys.Gamma)
...
num_grid.stance_FLRL = 5;
  num_grid.stance_RL = 10; 
num_grid.stance_RRRL = 5;
  num_grid.stance_RR = 10;
     num_grid.flight = 5;
nlp = HybridTrajectoryOptimization('A1_RGE',sys,num_grid,[],'EqualityConstraintBoundary',1e-3);
...
nlp.Phase(7).Plant.UserNlpConstraint  = @NLPSetUp.Constraints.FLRLConstraint;    
