...
model_bounds.states.x.ub(1:3) = [+10,+0.5, 3];

% torso [yaw,pitch,roll] position %
model_bounds.states.x.lb(4:6) = deg2rad([-2,-50,-50]);
model_bounds.states.x.ub(4:6) = deg2rad([+2,+50,+50]);
...
model_bounds.states.x.ub([8 11 14 17]) = deg2rad(+240); % 240

% calf joint angles -154.5 deg (shortest) to -52.5 deg (longest)
model_bounds.states.x.lb([9 12 15 18]) = deg2rad(-150); % -154.5
