...
if LOAD
    ...
    [sys, domains, guards] = load_behavior(robot, LOAD_PATH,'type','TGallopingGE');
else
    robot.configureDynamics('DelayCoriolisSet',OMIT_CORIOLIS,'OmitCoriolisSet',OMIT_CORIOLIS); 
    [sys, domains, guards] = load_behavior(robot,[], 'type','TGallopingGE');
end
%plot(sys.Gamma)
...
num_grid.stance_RRRL = 8;
  num_grid.stance_RL = 8;
     num_grid.flight = 5;
nlp = HybridTrajectoryOptimization('A1_TGE',sys,num_grid,[],'EqualityConstraintBoundary',1e-3);
